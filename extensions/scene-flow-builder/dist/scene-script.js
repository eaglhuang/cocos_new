...
function makeFullscreenNode(parent, name) {
    const n = getOrCreate(parent, name);
    const tf = n.getComponent(cc_1.UITransform) ?? n.addComponent(cc_1.UITransform);
    tf.setContentSize(1280, 720);
    const w = n.getComponent(cc_1.Widget) ?? n.addComponent(cc_1.Widget);
    w.isAlignTop = true;
...
function makeLabel(parent, name, text, fontSize, color, x, y) {
    const n = getOrCreate(parent, name);
    n.getComponent(cc_1.UITransform) ?? n.addComponent(cc_1.UITransform);
    const lbl = n.getComponent(cc_1.Label) ?? n.addComponent(cc_1.Label);
    lbl.string = text;
    lbl.fontSize = fontSize;
    lbl.color = color;
    n.setPosition(new cc_1.Vec3(x, y, 0));
    return n;
}
/**
 ...
 */
function makeButton(parent, name, labelText, x, y, width, height, targetNode, componentName, handlerName) {
    const n = getOrCreate(parent, name);
    const tf = n.getComponent(cc_1.UITransform) ?? n.addComponent(cc_1.UITransform);
    tf.setContentSize(width, height);
    const spr = n.getComponent(cc_1.Sprite) ?? n.addComponent(cc_1.Sprite);
    spr.color = new cc_1.Color(40, 60, 160, 220);
    const btn = n.getComponent(cc_1.Button) ?? n.addComponent(cc_1.Button);
    // 自動綁定 Click Event（等同 Unity Inspector 拖入 Button.onClick）
    if (targetNode && componentName && handlerName) {
        const handler = new cc_1.EventHandler();
    ...
    if (!lblNode) {
        lblNode = new cc_1.Node('Label');
        lblNode.layer = n.layer;
        lblNode.addComponent(cc_1.UITransform);
        const lbl = lblNode.addComponent(cc_1.Label);
...
exports.methods = {
    ...
    buildLoginScene() {
        ...
        bgSpr.color = new cc_1.Color(20, 25, 50, 255); // 深藍底色（佔位，之後換圖）
        // 標題
        makeLabel(canvas, 'TitleLabel', '三國傳承', 72, new cc_1.Color(220, 180, 60), 0, 120);
        makeLabel(canvas, 'SubtitleLabel', 'GLOBAL EXPEDITION', 28, new cc_1.Color(180, 180, 180), 0, 50);
        // 開始按鈕 — Click Event 自動綁定 LoginScene.onClickStart
        makeButton(canvas, 'BtnStart', '開始遊戲', 0, -80, 320, 70, scriptNode, 'LoginScene', 'onClickStart');
    ...
    buildLoadingScene() {
        ...
        makeLabel(canvas, 'LoadingHintLabel', '載入中，請稍候...', 32, new cc_1.Color(200, 200, 200), 0, -200);
        // 進度文字佔位（由 LoadingScene.ts 更新）
        makeLabel(canvas, 'ProgressLabel', '0%', 24, new cc_1.Color(150, 220, 255), 0, -250);
    ...
    buildLobbyScene() {
        ...
        const bg = makeFullscreenNode(canvas, 'Background');
        const bgSpr = bg.getComponent(cc_1.Sprite) ?? bg.addComponent(cc_1.Sprite);
        bgSpr.color = new cc_1.Color(15, 20, 25, 255);
        // 標題 (靠左偏)
        makeLabel(canvas, 'TitleLabel', '大廳主選單', 48, new cc_1.Color(220, 180, 60), -450, 320);
        // ── 功能按鈕群 (左對齊) ────────────────────────────────────────────────
        makeButton(canvas, 'BtnGeneralList', '武將列表 (Tab)', -450, 200, 260, 65, scriptNode, 'LobbyScene', 'onClickGeneralList');
        ...
        const listBg = listPanel.getComponent(cc_1.Sprite) ?? listPanel.addComponent(cc_1.Sprite);
        listBg.color = new cc_1.Color(10, 10, 10, 220);
        listPanel.active = false;
        // 面板標題 (靠左)
        makeLabel(listPanel, 'PanelTitle', '武將資料總覽', 30, new cc_1.Color(255, 220, 100), -300, 280);
        // 卡片容器 (靠左)
        const cardContainer = getOrCreate(listPanel, 'CardContainer');
        const ccTf = cardContainer.getComponent(cc_1.UITransform) ?? cardContainer.addComponent(cc_1.UITransform);
        ccTf.setAnchorPoint(0, 0.5);
        ccTf.setContentSize(600, 500);
        cardContainer.setPosition(new cc_1.Vec3(-450, 20, 0)); // ★ 與按鈕對齊
        const layout = cardContainer.getComponent('cc.Layout') ?? cardContainer.addComponent('cc.Layout');
        if (layout) {
        ...
        const dpTf = detailPanel.getComponent(cc_1.UITransform) ?? detailPanel.addComponent(cc_1.UITransform);
        dpTf.setContentSize(620, 480);
        detailPanel.setPosition(new cc_1.Vec3(0, 0, 0));
        const dpBg = detailPanel.getComponent(cc_1.Sprite) ?? detailPanel.addComponent(cc_1.Sprite);
        dpBg.color = new cc_1.Color(20, 30, 70, 245);
        ...
        labelDefs.forEach(([name, placeholder, size, y]) => {
            makeLabel(detailPanel, name, placeholder, size, new cc_1.Color(230, 230, 230), 0, y);
        });
        // 關閉按鈕
        makeButton(detailPanel, 'BtnClose', '✕ 關閉', 250, 200, 110, 46, detailPanel, 'GeneralDetailPanel', 'hide');
