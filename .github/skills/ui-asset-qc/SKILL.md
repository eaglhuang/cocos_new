---
name: ui-asset-qc
description: 'UI 資產品質驗證 SKILL — 執行 validate-visual-assets.js（R1-R6 規則）和 validate-ui-specs.js 對 skin family 資產、screen JSON、proof contract 進行全面驗證，輸出 QC 報告，標記 blocker/warning。USE FOR: 任何 UI 資產生成後、在 ui-preview-judge 前必須先通過的 QA 關卡。DO NOT USE FOR: debug runtime 崩潰（用 cocos-log-reader）、Layout 佈局調整（用 ui-vibe-pipeline）。'
...
| R6 | proof-contract-slot-coverage | warn | screen JSON 的 slots 應100%被 proof contract 覆蓋 |
...
| `reportPath` | 輸出報告路徑（預設：`artifacts/ui-qa/{id}-qc-report.json`）|
...
node tools_node/validate-visual-assets.js \
  --family {familyId} \
  --strict \
  --report artifacts/ui-qa/{screenId}-qc-report.json \
...
> 若 validate-ui-specs.js 尚未支援 `--check-content-contract`，改用 proof contract 手動比對（見 Step 4）。

### Step 3 — 讀取報告

```
read_file { filePath: "artifacts/ui-qa/{screenId}-qc-report.json" }
```
...
### Step 4 — Proof contract slot 覆蓋驗證（R6 手動）
...
比對 proof 的 `contentSlots[*].id` 是否全在 screen JSON 的 `slots` 欄位中出現。
若有缺漏，列出 missing slot IDs。
