...
> **2026-04-26 覆寫裁定：戰場 HUD 軍勢顯示**
>
> 1. 戰場 HUD 需顯示 `軍勢 Current / Sortie_Max`、`糧草 Current / Max`、`計謀點 Current / Max`、`場上部隊 Current / Limit`。
> 2. 虎符 / 部隊卡顯示 `投入兵力` 與 `糧草消耗`，例如 `投入 1000 兵 / 消耗 8 糧`。兵力不足、糧草不足、場上部隊已滿需各自有明確禁用態與 Toast。
> 3. 場上單位的生命條以 `Current_Troops / Unit_Troop_Cost` 顯示。退場返還剩餘兵力，被打光則全額計入戰損。
> 4. 戰報顯示 `出征軍勢`、`返還兵力`、`本戰損耗`、`城市兵源 Before -> After`、`補兵速度` 與 `預估恢復回合`；不再以抽象戰備損耗作為玩家主文案。
> 5. 後勤官摘要顯示軍糧上限與開局補給；參謀官摘要顯示計謀點上限與開局補點。
...
- **元件構成**:
  ...
  2. **糧草顯示 (Food Display)**: 
    ...
    - 若本場編入後勤官，需在同區或其旁顯示 `後勤官補給 +X` 摘要；後勤官加成影響的是戰場「目前 / 上限」兩個值，不是額外道具。
    - 回合開始時糧草**不自動增長**，與傳統 DP 每回合增長機制不同。
    - 若該場景有虎符部署摘要，可在同區顯示 `精銳上限 / AI 保留 band / Set 觸發` 等 1~2 個重點戰術提示。
    - 數值變動時需有跳動或顏色變化效果。
  3. **出征支援摘要 (Sortie Support Summary)**:
    - 以 1~2 個 chip 顯示本場 `參謀官` 與 `後勤官`。
    - 參謀官 chip 至少顯示 `姓名 / 計謀點上限加成 / 開局補點`。
    - 後勤官 chip 至少顯示 `姓名 / 軍糧上限加成`。
    - 這兩個 chip 屬摘要資訊，不代表場上多出兩個可直接操作單位。
  4. **陣地血量條 (Fortress HP)**:
...
- **小圓 2：計謀 (TacticsBtn)**:
  ...
  - 若本場編入參謀官，按鈕或其鄰近區域需同步顯示 `目前計謀點 / 上限` 與 `參謀官 +X`；推薦格式如 `計謀 21 / 48（參謀官 +9 / 上限 +18）`。
- **小圓 3：單挑 (DuelBtn)**:
...
### 6.3 參謀官智謀能量 HUD（場景戰法專用）

- 棋盤格上方狀態欄常駐顯示參謀官頭像 + 「INT 99, 正在執行: 夜襲」文字
...
- `strategist-hud-summary`
  ...
  - 內容：`Theme_Hazard`、`Interactive_Object_Status`、`目前情報值 / requiredIntelValue`、`Environment_Bonus_Pct`、`Recommended_Loadout_Gap`、`Strategist_Name`、`Current_Tactic_Points / Max_Tactic_Points`、`Strategist_Bonus`、`Deputy_Name`、`Current_Food / Max_Food`、`Deputy_Food_Cap_Bonus`。
...
## 7. 版本修訂記錄
- **2026-04-22 v6**: 正式把 `參謀官 / 後勤官` 納入戰場 HUD 規格；Top HUD 與計謀按鈕需顯示計謀點支援、軍糧上限支援與角色摘要。
