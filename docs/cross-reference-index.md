...
**最後更新**: 2026-04-05 (第三十二批：TigerTally runtime binding)

> **第三十二批整合（2026-04-05）**：`UI-2-0034` 已完成 runtime 收斂。`TigerTallyPanel` 現在會對 `ArtBg*`、`RarityBorder*`、`UnitTypeBadge*` 依資料動態載入 SpriteFrame，並沿用 `UI-2-0031` 的 fallback 契約：card art 缺件退回 `tally_card_art_placeholder`、rarity 缺件退回 `tally_rarity_normal`、type badge 缺件退回 `tally_badge_type`。同時 runtime 已預留 `artResource / rarityResource / typeBadgeResource` 欄位，後續正式卡圖與 F7 badge family 只需補素材，不必再改 TigerTally 載入鏈。
> **第三十一批整合（2026-04-05）**：`UI-2-0033` 已完成 runtime 收斂。`BattleScene` 現在會在初始化與 restart 時明確把 `playerGeneralId / enemyGeneralId` 傳入 `BattleHUD`，而 `BattleHUD` 會依 `sprites/generals/{snake_case_id}_portrait` 動態載入肖像，缺件時安全退回 player/enemy placeholder。這代表 HUD 頭像已脫離 skin manifest 的固定 placeholder 綁死，後續 `UI-2-0043` 只需聚焦在 crop proof 與視覺 QA，而不是再補 runtime 載入鏈。
> **第三十批整合（2026-04-05）**：`GeneralDetailBloodlineV3` 已將 `crest glyph family` 接入內容契約，新增 `CrestGlyphPrimary / CrestGlyphSecondary` 節點與 schema 欄位，並在 `preview-compare-2026-04-05-v7`、`v8` 截圖中驗證 glyph 與人物 glow 收斂有效。現階段可確認畫面已從「結構正確」進到「氣質接近參考圖」，剩餘最大缺口集中在專屬 `glyph / 命紋圖騰` 美術資產，而不是 layout 或資料鏈。
