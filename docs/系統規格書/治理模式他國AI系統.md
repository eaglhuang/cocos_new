...
function AI_ManageGenerals(nation: AINation): void {
    ...
    for (const gen of nation.generals) {
        if (gen.age > gen.lifespanMax || gen.vitality <= 0) {
            retireGeneral(gen, nation);
            // 產出虎符（同玩家規則）→ AI 自動裝備給最適合的武將
            // 裝備優先級：兵種適性 S > A > B > C
            // 退役武將 30% 機率流入傭兵市場
            if (Math.random() < 0.3) {
...
function decayRelation(relation: Relation): void {
    // 每季邦交值向 0 回歸 ±2
    if (relation.value > 0) relation.value = Math.max(0, relation.value - 2);
    if (relation.value < 0) relation.value = Math.min(0, relation.value + 2);
}
```
