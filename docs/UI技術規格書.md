...
> **參考來源**: `keep.md` (doc_index_0011)、`UI 規格書.md` (doc_ui_0027)、`主戰場UI規格書.md` (doc_ui_0001)、`美術素材規劃與使用說明.md` (doc_art_0003)
> **資產基準**: 所有 UI 相關資產目錄、命名與 legacy 例外，統一以 `docs/美術素材規劃與使用說明.md (doc_art_0003)` (doc_art_0003) 為準。
...
- 若 UI 需要新增實體圖塊、icon、底框、字型或 atlas 規則，必須先對齊 `docs/美術素材規劃與使用說明.md (doc_art_0003)` (doc_art_0003) 的正式路徑與命名規則。
...
> 完整矩陣與 Title 分類規則見 → **`docs/ui/component-sizing-contract.md (doc_ui_0038)` (doc_ui_0038)**（唯一尺寸真相來源）。
...
---
`docs/UI技術規格書.md (doc_ui_0049)` (doc_ui_0049) 確定了目前專案健康且穩健的實作方向。接下來的任何 UI 工作，皆須依照此標準，從「實作純資料與排版的 Prefab」開始堆疊遊戲畫面。
...
修改任何 fragment 前必須遵守五步防護流程（詳見 `.github/instructions/fragment-guard.instructions.md (doc_ai_0012)` (doc_ai_0012)）：
