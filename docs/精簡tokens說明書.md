...
### Hook 1：SessionStart — 自動注入 keep.summary.md
...
**行為**：
- 讀取 `docs/keep.summary.md` 的完整內容（33 行）
- 把內容包成 `additionalContext` 注入 Agent 的初始上下文
- 同時附加禁止呼叫 `get_changed_files` 的紅色警告

**效果**：Agent 不需要主動讀取 keep.summary.md，也能在對話開始時就知道：
- 各 shard 目錄路徑
- pre-flight 必讀規則
- 禁止行為清單
...
注入訊息：
```
⚠️ Token 警告：keep.md 是大型重量檔案（估算 >6000 tokens）。
建議替代方式：
...
🚨 編碼防災警告：以下檔案可能有 UTF-8 BOM 或亂碼（mojibake）：
  docs/keep-shards/keep-workflow.md

請立刻執行修復：
  node tools_node/check-encoding-touched.js --files docs/keep-shards/keep-workflow.md
...
|-----------|------|---------|---------|
| `SessionStart` | `session-start.js` | 注入 keep.summary.md + 禁止提示 | 軟注入（always inject） |
...
| Pre-flight 入口 | keep.md 全文 | keep.summary.md（33 行）| ✅ SessionStart 自動注入 |
